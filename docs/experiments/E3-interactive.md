...
| 7 | **`/rename` propagates** | In A: `/rename alice-renamed`, wait ~30 s (one heartbeat), then in B: "Run `brigade sessions`." | B's listing shows A under the new name (the watcher re-reads the registry each heartbeat). | **PASS, with the timing BOUNDED not measured** (§1d). After `/rename`, the **team's own roster** showed the new name (`e3-renamed-0e8e`, state `idle`). Verified by reading the fs store from outside any session rather than from a second session — the same record B would have read. **How long it took is not established**: the driver waited 70 s (deliberately two heartbeat windows) and then took a *single* roster read, so every value from one heartbeat to 70 s is consistent with this observation, and one run cannot separate them. The propagation is proven; its latency is not. |
...
| 15 | **A third first-run interruption** (sitting correction 5) | Note any onboarding prompt (`Claude in Chrome extension detected`, the trust dialog, the fullscreen-renderer write) that appeared before the session prompt in any of the runs above. | Recorded, for P4-3/P4-5's `expect` drivers. | **Recorded across 44 driven sessions** (§1d, `onboarding.py`, re-run after the last arm landed). The **trust dialog** appeared in **36/44** — every genuinely fresh directory, and never in a directory already trusted (the repository, or a temp directory being re-opened). The **`Claude in Chrome extension detected` prompt appeared 0/44** on 2.1.259; E0-8 met it on 2.1.252, so an `expect` driver must still tolerate it but it is not currently raised here. No other onboarding prompt matched. One session needed a canary retry, on the narrow-pty driver since fixed. *(An earlier version of this row read 21/29. That was a correct tally taken at 14:53 EDT, before the last fifteen runs; it was never recomputed. Re-running the named script is the authority, and it now prints 44.)* |
...
| 15 first-run interruptions | all | 44 | trust dialog 36/44, Chrome prompt **0/44** |
...
- **Check 7 proves propagation, not latency.** One roster read after a fixed 70 s wait cannot distinguish one
  heartbeat from seventy seconds. If the heartbeat interval matters to a later task, measure it with repeated
  reads rather than citing this row.
- **Check 15's tally moves as runs are added.** `onboarding.py` counts every run directory under
  `.ignored/e3-interactive/`, so a re-run after more arms land gives a larger denominator. The figure here is
  44 sessions; re-run the script rather than quoting a number from prose.
