...
**Result of follow-up (1), 2026-09-03, Claude Code 2.1.259:** repeated from a fresh temporary directory — no Skill dialog,
the skill used successfully. So the missing dialog is a behaviour change since E0-8 (2.1.252), not stored state: the
skill's `allowed-tools` grant now costs a Manual-mode user NO prompt at all for the turn that invokes the skill. D20
stands (the grant is the mechanism); its measured cost improved from one dismissible dialog per project to none.
The plugin README's permissions paragraph and the execution log carry the correction; the sitting's note that a
mismatched `allowed-tools` pattern "raises the dialog too" is untested on 2.1.259.
