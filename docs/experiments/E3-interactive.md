...
| 2 | **The grant does not outlive the turn** | In A, next prompt, without invoking the skill: "Run `brigade sessions` again." | A Bash permission prompt appears (the grant cleared when you sent the next message). | No prompt, team-messaging use successful|
