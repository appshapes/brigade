...
| `E3-interactive.md` | P3-8. **Checks 1 and 2 are settled, and they did not need a keyboard**: an `expect` pty in Manual mode measures them, and `scripts/experiments/E3-interactive/` is the driver. Twelve interactive sessions on 2.1.259, permission mode confirmed `default` four independent ways in every one: **one Skill dialog, then the skill's bare `brigade` commands run with no Bash prompt (6/6), and the next turn prompts (6/6)** — E0-8 (b)'s 2.1.252 result unchanged. The dialog's option 2 is scoped to the directory it names (dismiss → same directory silent → fresh directory raises it again), and the null control stalled 3/3. Five defects in the harness were found adversarially and fixed *before* its results were believed, the decisive one being that the Skill and Bash dialogs share the words "Do you want to proceed?" on 2.1.259. Checks 3–15 remain open. |
