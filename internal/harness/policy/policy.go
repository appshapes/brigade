...
//
// D18 settles the shape: the policy is `accept` or `refuse` and NOTHING
// else. `accept` is the default in every permission mode and entrypoint —
// bypassPermissions, auto, `claude -p` workers included — so a team
// message is injected immediately and nothing waits for a human. `refuse`
// is the opt-in (the `team_inbound` option), and it also applies whenever
// the option asked for something this release cannot honour (`hold` is
// Phase 5, P5-9; an unknown spelling fails closed) or whenever the native
// settings scan found a `crossSessionInbound` of `hold` or `refuse`. The
// scan is load-bearing, not defensive (E0-9): under a native `refuse` a
// socket post SUCCEEDS and the frame vanishes, so a Brigade `accept` there
// would acknowledge messages Claude never saw — the message gone AND the
// sender told it arrived. `permission_mode`, `non_interactive` and the
// entrypoint are recorded for diagnostics only; Decide echoes them and a
// test table proves they never touch the outcome.
package policy
...
// A Policy is the effective inbound policy: Accept or Refuse. `hold` is
// never a value of this type in this phase (D18, P5-9).
type Policy string
...
// Valid reports whether p is one of the two policies this phase honours.
func (p Policy) Valid() bool { return p == Accept || p == Refuse }

// WarnOptionUnknown is attached when the option value handed to Effective is
// neither Accept nor Refuse — a caller bug (config.ParseInbound never
...
// Effective computes the policy (6.8 step 3, 6.10): Refuse when the option
// is refuse, when the option was `hold` or invalid (optionWarning is the
// config.ParseInbound warning that says so, passed through unchanged so the
// hook prints it), or when the native scan found `hold` or `refuse` (the
// scan's own warning is appended); otherwise Accept. The warnings come back
// in that order, each a fixed text that never echoes a setting value the
// user did not choose from the two known ones.
...
type Inputs struct {
	...
	// env's), "" when the option was accept or refuse.
