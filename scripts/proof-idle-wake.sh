...
# ---- end frame literals ----
# The frame level the receiving sessions run at (P5-12): --frame <open|guarded|strict> sets it and launch() passes it
# as the `frame` plugin option; with no flag no frame option is passed and the shipped default is what is proved.
...
while [ $# -gt 0 ]; do
  case $1 in
    ...
    *) die "unknown argument: $1 (usage: proof-idle-wake.sh [--wakes n] [--long-idle s] [--skip-control] [--skip-multi] [--resume stamp] [--frame open|guarded|strict] | wake <dir>)" ;;
  esac
done
...
say "proof-idle-wake.sh: frame    $frame_level$(if [ "$frame_set" = yes ]; then printf ' (--frame, passed as the frame plugin option)'; else printf ' (the shipped default; no frame option is passed)'; fi)"
