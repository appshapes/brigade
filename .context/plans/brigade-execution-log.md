...
| P6-1..P6-5 | **House conventions**: adapt CI workflows, `Makefile` targets, `scripts/` and the test harnesses to the owner's usual practice (see "Phase 6" below) | **P6-1 done** (`docs/research/house-conventions.md`); **P6-2..P6-5 unblocked 2026-09-04** — the owner answered all seven questions (recorded under "Phase 6"); run after P5-13 | Opus | the answers: tag-based release stays, `release` keeps its name, CI steps stay separate, `docs/` visible + a task-table README TOC, bare-noun Makefile groups; plus the two housekeeping items (tracked `.pyc` files; the dead Docker group and the `help` scrape hiding `e2e`) |
