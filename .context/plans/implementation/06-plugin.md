...
| Display name | registry `$CLAUDE_CONFIG_DIR/sessions/<pid>.json` `name` when its `nameSource` is not `derived` (undocumented format, best effort) → the transcript's latest `custom-title` record (the VS Code extension's rename, which leaves the registry name derived; an empty one means no title) → the derived registry `name` (`<basename(cwd)>-<hex>`) → hook `session_title` → `basename(cwd)`; one rule, `registry.ResolveName`, in the hook and the watcher | registered as `session_name`; re-resolved every heartbeat so `/rename` and a VS Code rename propagate |
