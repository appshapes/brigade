...
- 2026-09-05 — "P5-9 DONE" — 3.6 and the state-file table: the pending file is `state/pending/<StateName>.json` (P5-14's stem) carrying id, sender name, `sender_session_id`, summary and time — never the body; the release file is `state/release/<StateName>.json`, written and merged by the command, compare-then-deleted by the watcher after the pending save; the release rides the existing liveness tick plus once at `ready` (no supervisor stdin channel — the watcher's stdin is nil and the stdin commands go to the adapter child); the held notice is `inbound.HeldNotice`'s text (≤3 sanitised names + count). `hold → accept` through the map is not a release; the terminal listing shows released-but-undelivered entries as waiting for the watcher.
