...
- 2026-09-05 — "P5-1 DONE" — 5.9 hosted steps 1–2: `supabase config push` is refused for this repository (it pushes the LOCAL `config.toml` whole — `rate_limit_anonymous_users` 1000 against the hosted 30/h/IP and `site_url` `http://127.0.0.1:3000`; `[remotes.<name>]` is the vehicle if it is ever wanted); the hosted settings are set field by field through the Management API by `scripts/backend-settings.sh`; no database password is needed on CLI 2.116.0 (`link` ignores it, `db push --linked` mints a temporary login role with the PAT). Measured on the hosted project: four migrations (not three), `db_schema` stored as `public,graphql_public,brigade` (no space), the two session-timeout fields read `0` not `null`, the Realtime upgrade probe needs `curl --http1.1`, and the `PrivateOnly` reason is capitalised `This project only allows private channels` (only the prefix is matched).
- 2026-09-05 — "P5-1 DONE" — 5.6: "Allow public access" is the Management API field `private_only` on `PATCH /v1/projects/{ref}/config/realtime`, set by P5-1 through the API; the dashboard route is documentation for a human, not the mechanism. 5.8/D13: pg_cron 1.6.4 IS present on the Free plan and both jobs are scheduled — no degrade to opportunistic-only gc. The threat model's "30 per hour per IP, not configurable" is wrong on the second clause: `rate_limit_anonymous_users` is settable (minimum 1); the default stands and P5-1 leaves it alone.
- 2026-09-05 — "P5-11 DONE" — 5.1: the flock lives in `internal/adapterkit/flock.go` (there is no `lock.go`); the lock retry is **5 ms** (`lockRetryInterval`, E0-6's fix, P1-3), not the 100 ms `LOCK_NB` poll 5.1 still prescribes; and "a token two or more steps behind revokes the family" is wrong — measured again twice on GoTrue v2.196.0: a two-behind redemption answers 400 `refresh_token_already_used` and flags every row `revoked`, yet the leaf still refreshes and the family survives (the code comment at `credentials.go:26-28` already says so). Two watchers sharing one `session.json` for 2 h at the shipped `jwt_expiry = 3600`: 2 rotations, exactly 2 `/token` calls (the second adopted under the lock), 0 `lock_timeout`.
- 2026-09-05 — "P5-11 DONE" — 5.6 / 5.11: `profile status` takes no `--json` (`usage`, exit 2) — the adapter's output is always protocol JSON, so any document writing `profile status --json` is wrong; and the adapter logs **nothing** on a successful drain at any level and nothing at INFO on a refresh or an adoption (0 bytes in 2 h), so a soak reads drains from `pg_stat_statements`, rotations from `profile status` and GoTrue's audit trail, never from the adapter log. `onHint` coalescing measured: 10 hints → 2 drains, 1,000 hints → +43 drains (21× fewer than hints).
