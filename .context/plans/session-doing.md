...
**Status.** Brief, **v3.1, shipped through P16-6; P16-7 (acceptance) pending** — rows P16-0..P16-6 landed on
...
needed"; the §5.2 dagger stands). **The one-time resolution of a pre-feature map serves the next prompt**, not
...
items 27–30** expect `ignore`, `ignore`, `ask`, `ignore` and are measured in P16-7, outside the 26-item measured
set the docs still cite as history; `proof-headless.sh` reads its count from `expected.json` and its judge's
...
**Still unmeasured, and owned by a row:** P16-0's unmeasured items (an inbox-socket wake-up; `auto`'s
classifier) and whether models act on the three strings — all P16-7.
