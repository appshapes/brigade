-- Brigade session brigade-version migration (capability session.brigade_version; C-46). Applied after
-- 20260917170000_session_human_label.sql. It carries ONE fact onto brigade.sessions: the version of the Brigade
-- harness a session is running, as that harness reports it (docs/protocol-v1.md 4.4.2 `brigade_version`, at most 64
-- code points, unverified display text) — NOT its host's version, which harness_version has always carried. It goes
-- into brigade.session_record (so `session register` and `session list` return it, 4.4.3) and through
-- register_session and session_heartbeat (4.4.4: absent means unchanged, the harness never clears it). A heartbeat
-- carries it because a harness can be replaced by a newer one while its session lives — `/reload-plugins` does
-- exactly that to the watcher — and a roster should name the version that is running.
--
-- Why it exists: nothing on the wire said which Brigade a teammate's session ran, so nobody could tell from the
-- roster who was behind a release. A session whose harness predates this member never sends it and reads as null,
-- which a roster shows as a blank cell — itself the answer "older than the release that added this".
--
-- Migrations are append-only once applied (`supabase db push` tracks them by file name), so no earlier file is
-- edited: register_session below repeats 20260917170000's body, and session_heartbeat and session_record repeat
-- 20260910193200's, byte for byte apart from the appended parameter, its validation and the column in the insert,
-- the resume update, the heartbeat's coalesce() list and the record.
--
...
--
-- The cap is a wire-format bound with no `limits` member (protocol.MaxBrigadeVersionChars: `limits` members are
-- required, so a new one would fail `describe` for every adapter written before it). It is checked twice, as
-- model's is: the RPC raises the D15 text 'brigade:invalid_input:brigade_version' (errcode 22023, so the adapter
-- answers invalid_input with details.field = brigade_version, C-46), and the column constraint is the belt beneath.

alter table brigade.sessions
  add column brigade_version text check (char_length(brigade_version) <= 64);
...
drop function brigade.register_session(uuid, text, text, text, text, text, text, text, integer, uuid, text, bigint, text);
create function brigade.register_session(
  ...
  p_brigade_version text default null)
...
begin
  ...
  if p_resume_session_id is not null then
    ...
    insert into brigade.sessions (team_id, owner_id, name, description, activity, inbound, harness, harness_version, workspace_label, lease_seconds,
                                  model, context_used_tokens, brigade_version)
    values (p_team_id, v_uid, p_name, p_description, p_activity, p_inbound, p_harness, p_harness_version, p_workspace_label, p_lease_seconds,
            p_model, p_context_used_tokens, p_brigade_version)
...
revoke execute on function brigade.register_session(uuid, text, text, text, text, text, text, text, integer, uuid, text, bigint, text, text) from public, anon;
grant  execute on function brigade.register_session(uuid, text, text, text, text, text, text, text, integer, uuid, text, bigint, text, text) to authenticated;

drop function brigade.session_heartbeat(uuid, text, text, text, text, integer, text, bigint);
...
begin
  ...
  update brigade.sessions
     set last_seen_at = now(), activity = coalesce(p_activity, activity), name = coalesce(p_name, name),
         ...
         brigade_version = coalesce(p_brigade_version, brigade_version)
...
revoke execute on function brigade.session_heartbeat(uuid, text, text, text, text, integer, text, bigint, text) from public, anon;
grant  execute on function brigade.session_heartbeat(uuid, text, text, text, text, integer, text, bigint, text) to authenticated;
